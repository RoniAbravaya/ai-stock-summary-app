...
router.get('/', async (req, res) => {
  try {
    ...
    if (process.env.ENABLE_MOCK_DATA === 'true') {
      ...
      const mockStocks = mockData.getStocksData();
      return res.json({
        success: true,
        data: mockStocks,
        source: 'mock',
...
router.get('/trending', async (req, res) => {
  try {
    ...
    if (process.env.ENABLE_MOCK_DATA === 'true') {
      ...
      const mockStocks = mockData.getStocksData().filter(stock => stock.trending);
      return res.json({
        success: true,
        data: mockStocks,
        source: 'mock',
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (process.env.ENABLE_MOCK_DATA === 'true') {
      ...
      const mockStocks = mockData.getStocksData().filter(stock => 
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (process.env.ENABLE_MOCK_DATA === 'true') {
      ...
      const mockStock = mockData.getStocksData().find(stock => 
        stock.symbol.toUpperCase() === ticker
      );
      if (mockStock) {
        return res.json({
          success: true,
